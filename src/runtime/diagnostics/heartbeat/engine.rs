//! Heartbeat engine: periodically reads `HEARTBEAT.md` and executes
//! parsed tasks on a configurable interval.
...
impl HeartbeatEngine {
    /// Create a new heartbeat engine with the given config and observer.
    pub fn new(
        config: HeartbeatConfig,
        workspace_dir: std::path::PathBuf,
        observer: Arc<dyn Observer>,
    ...
    ///
    /// # Errors
    /// Returns an error when task collection or heartbeat file I/O fails.
    ...
    ///
    /// # Errors
    /// Returns an error if reading `HEARTBEAT.md` fails.
    ...
    ///
    /// # Errors
    /// Returns an error if creating directories or writing `HEARTBEAT.md` fails.
...
mod tests {
    ...
    async fn ensure_heartbeat_file_creates_file() {
        let dir = std::env::temp_dir().join("asterel_test_heartbeat");
    ...
    async fn ensure_heartbeat_file_does_not_overwrite() {
        let dir = std::env::temp_dir().join("asterel_test_heartbeat_no_overwrite");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn tick_returns_zero_when_no_file() {
        let dir = std::env::temp_dir().join("asterel_test_tick_no_file");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();

        let observer: Arc<dyn Observer> = Arc::new(crate::contracts::observability::NoopObserver);
    ...
    async fn tick_counts_tasks_from_file() {
        let dir = std::env::temp_dir().join("asterel_test_tick_count");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
