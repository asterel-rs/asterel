    ...
    pub(super) fn enforce_response(self) -> (StatusCode, Json<serde_json::Value>) {
        match self {
            Self::KillSwitchEnabled => problem_response(
                ...
                "kill_switch_enabled",
                "Service Unavailable",
                "Service unavailable — gateway defense kill switch is enabled.",
            ),
...
/// Returns the configured defense mode.
pub(super) fn effective_defense_mode(state: &AppState) -> GatewayDefenseMode {
    state.access.defense_mode
}

/// Returns a fail-closed response when the gateway defense kill switch is engaged.
pub(super) fn kill_switch_response(
    state: &AppState,
) -> Option<(StatusCode, Json<serde_json::Value>)> {
    state
        .access
        .defense_kill_switch
        .then(|| PolicyViolation::KillSwitchEnabled.enforce_response())
}
