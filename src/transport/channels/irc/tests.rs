//! Unit tests for IRC message parsing, SASL encoding, message splitting,
//! nickname handling, and channel creation.
...
fn irc_config_serde_roundtrip() {
    ...
    assert_eq!(parsed.username.as_deref(), Some("asterel"));
    ...
    assert!(parsed.security.autonomy_level.is_none());
    assert!(parsed.security.tool_allowlist.is_none());
}
