//! Unit tests for `WhatsApp` channel: name, verify token, number allowlist,
//! wildcard matching, and capability flags.
use super::*;
...
fn whatsapp_parse_valid_text_message() {
    ...
    assert_eq!(msgs[0].content, "Hello Asterel!");
    assert_eq!(msgs[0].channel, "whatsapp");
    assert_eq!(msgs[0].id, "wamid.xxx");
    assert_eq!(msgs[0].conversation_id.as_deref(), Some("+1234567890"));
    assert_eq!(msgs[0].message_id.as_deref(), Some("wamid.xxx"));
