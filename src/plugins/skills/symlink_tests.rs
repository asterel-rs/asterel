//! Symlink edge-case tests for skill directory resolution.
...
mod tests {
    ...
    use crate::plugins::skills::{load_skills_with_policy_and_config, skills_dir};
    use crate::security::SecurityPolicy;

    #[test]
    fn skills_symlink_unix_edge_cases() {
    ...
    #[test]
    fn skills_loader_ignores_symlinks_even_when_filesystem_can_read_them() {
        let tmp = TempDir::new().unwrap();
        let workspace_dir = tmp.path().join("workspace");
        std::fs::create_dir_all(&workspace_dir).unwrap();
        ...
        #[cfg(unix)]
        {
            // The OS may allow and resolve this symlink, but the skill loader's
            // policy is stricter: symlinked skill directories are not trusted
            // prompt sources, even when the target is readable.
            ...
            assert!(result.is_ok(), "test setup should create readable symlink");

            // Filesystem readability is not authorization for prompt loading.
            let content = std::fs::read_to_string(dest_link.join("SKILL.md"));
            assert!(content.is_ok());
            ...
            assert!(
                skills.is_empty(),
                "symlinked skill directories must be ignored by the loader"
            );
        }
    }
}
