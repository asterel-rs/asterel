-- Asterel PostgreSQL Memory Backend — Migration 007: GraphRAG extensions
--
-- Adds constrained GraphRAG metadata for canonical entity resolution,
-- ontology-aware extraction, and bitemporal graph edges.
...
-- NOTE:
-- `graph_entities.embedding` is intentionally left dimensionless (`vector`)
-- because its eventual width follows the configured embedding backend. pgvector
-- cannot build an HNSW index for a column without fixed dimensions, so we do
-- not create a graph-entity embedding index at migration time.
